        ...
        <div v-if="$store.state.Articlestore.cart.length == 0">
          <div class="cart-empty text-center">
            ...
            <div class="start-shopping">
              <router-link to="/shopping" class="btn btn-primary">
                Take me to Products Page
              </router-link>
              <span class="ml-2">Start Shopping</span>
            </div>
          </div>
        </div>
        <div v-else>
            ...
            <div v-for="(c) of $store.state.Articlestore.cart" :key="c.product.id">
              <div class="cart-item" key="{cartItem.id}">
                <div class="cart-product d-flex align-items-center">
                  <img :src="c.product.imageart" :alt="c.product.designation" class="mr-3" />
                  <div>
                    <h3>{{ c.product.designation }}</h3>
                    <p>{{ c.product.reference }}</p>
                    <button class="btn btn-danger" @click="removeFromCart(c)">
                      Remove
                    </button>
                  </div>
                </div>
                <div class="cart-product-price">{{ c.product.price }} TND</div>
                <div class="cart-product-quantity">
                  <button @click="minus(c)" v-if="c.qty > 1" class="btn btn-outline-secondary">
                    -
                  </button>
                  <div class="count mx-2"><strong>{{ c.qty }}</strong></div>
                  <button @click="plus(c)" class="btn btn-outline-secondary">+</button>
                </div>
                <div class="cart-product-total-price">
                  {{ c.product.price }} * {{ c.qty }} TND
                </div>
              </div>
            </div>
          </div>
          <div class="cart-summary mt-4">
            <button class="clear-btn btn btn-danger" @click="clearCart">
              Clear Cart
            </button>
...
.product {
  ...
  max-width: 100%;
  height: 400px;
  box-shadow: -5px -5px 10px rgb(145, 126, 126),
